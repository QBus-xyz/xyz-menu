$(document).ready(function () {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            buttons.length = 0
            submenus.length = 0
            $.post('https://xyz-menu/close', JSON.stringify({}));
    ...
    window.addEventListener('message', function (event) {
        ...
        if(item.addbutton == true) {
            ...
            if(item.menuid == false || null) {
                var b = (`<button id="normalbutton" data-type="`+item.type+`" data-trigger="`+item.trigger+`" data-param="`+item.param+`" data-uid="`+item.uid+`" class = "btn"><div class="title">`+title+`</div><div class="description" >`+description+`</div></button>`)
                $(".container").fadeIn(500)
                buttons.push(b)
                if( item.uid ) {
                    params.set(item.uid.toString(), item.param)
                }
                $(".container").append(b).fadeIn(400)
                return
            } else {
                var b = (`<button id="`+item.menuid+`" data-type="`+item.type+`" data-trigger="`+item.trigger+`" data-param="`+item.param+`" data-uid="`+item.uid+`" class = "a btn"><div class="title">`+title+`</div><div class="description" >`+description+`</div></button>`)
                $(".container").fadeIn(500)
                buttons.push(b)
                if( item.uid ) {
                    params.set(item.uid.toString(), item.param)
    ...
    $("body").on("click" , ".a" , function(){
        $.post('https://xyz-menu/clicked', JSON.stringify({type:$(this).attr("data-type"), trigger:$(this).attr("data-trigger"), param:params.get($(this).attr("data-uid")) }));
        $.post('https://xyz-menu/close', JSON.stringify({}));
        $(".container").fadeOut('slow')
        setTimeout(() => {
            $("button").remove()
